...
import { setupWebXR } from '../xr/setupWebXR';
...
export function createApp(): void {
  ...
  let healthBarsManager: HealthBarsManager | null = null;

  function removeHealthBar(ent: Entity): void {
    healthBarsManager?.remove(ent);
  }

  function updateHealthBars(): void {
    if (!healthBarsManager || !sim) {
      healthBarsManager?.removeAll();
      return;
    }
    healthBarsManager.update(sim.enemies);
  }
  ...
  // WebXR (button + controller visuals)
  setupWebXR(renderer, scene);
  ...
  healthBarsManager = createHealthBarsManager({
    healthLayer,
    camera,
    renderer,
    getVisual: (e) => visuals.get(e),
    getEntityBaseY,
  });
  ...
  function drawMinimap() {
    drawMinimapUi({ minimap, sim, player, targeting, gameMode, raceLoopPts, raceFinishA, raceFinishB });
  }
